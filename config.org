...
Tengo un proyecto personal en [[https://github.com/alvarogonzalezsotillo/grading-questionnaire]] que [[https://alvarogonzalezsotillo.github.io/blog/plantilla-latex-para-examenes/][convierte un fichero =gift= en un =pdf=]] para exámenes. Esta función me facilita la conversión.
   #+begin_src emacs-lisp
     (defun gifttolatex (titulo porcentajetest)
       ...
       (let
           ((comando (format "sh -c 'gifttolatex -k -t \"%s\" -q \"%s\" \"%s\"'" titulo porcentajetest buffer-file-name))
            )
...
#+begin_src emacs-lisp
  ...
  (defun region-occurrences-highlighter--accept (begin end)
    "Accept to highlight occurrences if less than the limit."
    (and
     (not (eq begin end))
  ...
  (defun region-occurrences-highlighter--change-hook ()
    "Called each time the region changes."
