...
#+begin_src emacs-lisp
  (setq org-babel-load-languages '((scala . t) (shell . t) (emacs-lisp . t) (dot . t)))
...
- Resaltar el símbolo bajo el cursor de forma dinámica. Además resaltarlo en todo el buffer, para que se pueda navegar por todas las ocurrencias del fichero.
#+begin_src emacs-lisp
...
*** /Child frame/
=helm= se muestra en una nueva ventana. Esta ventana puede estar en una nueva /child frame/ para no cambiar la disposición de la /frame/ original. En este momento tengo esta opción deshabilitada, ya que en algunos sistemas de ventanas es bastante lento.
#+begin_src emacs
...
Utilizo /git/ para casi todos mis ficheros. =git-gutter= marca en el margen izquierdo las líneas cambiadas, añadidas o borradas respecto de la versión de la rama actual.
#+begin_src emacs-lisp
