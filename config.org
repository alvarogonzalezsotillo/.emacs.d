...
#+RESULTS:
| addtopath.sh                 |
| arrancar-emulador-android.sh |
| arrancar-epoptes-en-clase.sh |
| bfg-1.13.0.jar               |
| clonar-2-de-3.sh             |
| clone-2.sh                   |
| colores-tty.sh               |
...
| iptables-save                |
| keymon.sh                    |
| monitoriza-logins.sh         |
| pdf2svg.sh                   |
| plantuml.1.2018.11.jar       |
| promt-en-zsh.sh              |
| quitar-red-a-usuario.sh      |
| reducir-video.sh             |
| rmtemplatex.sh               |
| rotate-screen.sh             |
| screen-to-serial.sh          |
...
(use-package org
  ...
  (defun my-org-export-deterministic-reference (references)
    (let ((new (length references)))
      (while (rassq new references) (setq new (+ new 1))) 
      new))
  (advice-add #'org-export-new-reference :override #'my-org-export-deterministic-reference)
...
(use-package helm
  ...
  (add-hook 'helm-major-mode-hook #'helm-bien)
...
** emojis
En alguna versión de emacs dejó de hacer falta
   #+begin_src lisp
   (use-package emojify
     :ensure t
...
(use-package blamer
  ...
  (setq blamer-bindings '(("<mouse-1>" . blamer-callback-magit-log-file)
                          ("<mouse-2>" . blamer-callback-show-commit-diff)
                          ("<mouse-3>" . blamer-callback-timemachine)))
  )
   #+end_src

** =magit-todo=
TODOMAGITTODO: Igual puedo automatizar los tags con
#+begin_src bash
ag --nofilename --nonumbers --silent -o "^[[:space:]]*(\;+[[:space:]]+|\#+[[:space:]]+|//+[[:space:]]+)?[[:space:]]*\K[A-Z][A-Z][A-Z]+(?=(\: ))" * | sort  |uniq
#+end_src
...
#+RESULTS:
|   | (:eval (propertize [☰] 'local-map (make-mode-line-mouse-map 'mouse-1 'menu-bar-open))) |   | ((:eval (mode-icons--read-only-status)) (:eval (mode-icons--modified-status))) | %[ | (%12b) | %] |   |   | (vc-mode vc-mode) |   | %m %n |   | %IB %Z |   | %l:%c %p | (:eval (unless (display-graphic-p) -%-)) |
...
** =same-window-regexps=
Los buffers que se abran que casen con estas expresiones se abren en las mismas ventanas
#+begin_src emacs-lisp
