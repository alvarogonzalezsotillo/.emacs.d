...
Para actualizar siempre los paquetes, utilizo =auto-package-update= 
#+BEGIN_SRC emacs-lisp 
  (use-package auto-package-update
    ...
    (setq auto-package-update-interval 9999)
...
| wgrep                          |                                                                |
| wgrep-helm                     |                                                                |
| which-key                      | Pistas acerca de los atajos de teclado                         |
...
Para imenu
#+BEGIN_SRC emacs-lisp
(add-to-list 'TeX-outline-extra '("\\\\PROBLEMA{" 4))
#+END_SRC
...
#+begin_src emacs-lisp
  ...
  (defun abrir-proyecto()
    ...
    (let ( (proyecto (abrir-proyecto--selecciona-proyecto) )
           (frame (make-frame)))

      (select-frame-set-input-focus frame)
      (message (concat "El proyecto seleccionado es:" proyecto ))
      (dired proyecto)
      (neotree-project-root-dir-or-current-dir)
      (magit-status)))
#+end_src
...
#+RESULTS:
: [Install fonts process still running, please call ’use-ttf-set-default-font’ after a while.]
