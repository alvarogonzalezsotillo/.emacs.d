...
- Los ficheros =org= dan problemas sin tienen un =title:= definido. Para solucionarlo:
#+begin_src sh
...
** Tema de inicio
Pongo uno detrás de otro para "limpiar" lo que haya podido quedarse de alguna customización.
#+begin_src emacs-lisp
(tema-claro)
(tema-oscuro)
#+end_src
...
- Me gusta que la tecla escape cancele cualquier acción. Con el siguiente código hago que se cancelen incluso más acciones que con =C-g=
#+begin_src emacs-lisp
(define-key company-active-map [escape] 'company-abort)
