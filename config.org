...
(let 
  (  
     ...
     (paths `(,(concat (expand-file-name user-emacs-directory) "bin") ,(concat (expand-file-name "~/") ".cargo/bin") ) )
  )
  (dolist (path paths)
    (message "path: %s" path)
...
(use-package helm
  ...
  :after (tramp helm-icons) ;; PARA EVITAR EL ERROR Symbols's value as variable is void: tramp-methods
  :bind
      (("M-x"  . helm-M-x))
      (("<menu>"  . helm-M-x))
...
Intento de colocar la ventana de helm siempre abajo, tiene problemas de redisplay
#+begin_src lisp
...
#+RESULTS:
|   | ((:eval (mode-icons--read-only-status)) (:eval (mode-icons--modified-status))) | %[ | (%12b) | %] |   |   | (vc-mode vc-mode) |   | %m %n |   | %IB %Z |   | %l:%c %p | (:eval (unless (display-graphic-p) -%-)) |
...
Utilizo /git/ para casi todos mis ficheros. =diff-hl=  marca en el margen izquierdo las líneas cambiadas, añadidas o borradas respecto de la versión de la rama actual. indico que se refresquen los buffers cada 10 segundos.

#+BEGIN_SRC emacs-lisp
(add-to-list 'load-path "/home/alvaro/github/diff-hl")
...
(use-package diff-hl-show-hunk
  :defer 1
  :config
d  (setq diff-hl-show-hunk-function #'diff-hl-show-hunk-popup)
...
- wrap-region
- stripe-buffer
- (use-package autorevert :ensure nil :config (setq auto-revert-verbose nil global-auto-revert-mode t global-auto-revert-non-file-buffers t))
