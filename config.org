...
** Paquete =swoop=
Búsqueda con previsualización. Lo configuro para que no seleccione el símbolo en en cursor.
#+begin_src emacs-lisp
(use-package swoop
  :ensure t
  :defer 1
  :config
    ;(setq helm-swoop-pre-input-function nil)
)
#+end_src
...
** Abrir proyecto en ventana nueva con neotree
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (defun click-en-el-margen (event) 
    ...
    (let*
      ...
      (message "Han hecho click en el margen %s" event)
  ...
  ;(condition-case nil
      (let*
          (
           ...
           (content (git-gutter-hunk-content info))
           (buffer (generate-new-buffer "git-gutter+:"))
           )
        (git-gutter:update-popuped-buffer (git-gutter:search-here-diffinfo git-gutter:diffinfos))
        (message "infos: %s" (git-gutter:search-here-diffinfo git-gutter:diffinfos))
...
(define-key global-map (kbd "<left-margin> <mouse-1>") 'kk)
#+end_src
